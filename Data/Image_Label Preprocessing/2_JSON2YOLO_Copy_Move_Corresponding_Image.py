...
# 0. 변수 정의
# ==============================================================
parser = argparse.ArgumentParser(description='2_JSON2YOLO_Copy_Move_Corresponding_Image')
...
parser.add_argument('--json-filenames', default=['instances_train2017.json', 'instances_val2017.json'], type=str, help='yolo로 변환할 json 파일명 지정 (단, json_mother_abs_path 폴더의 자식으로 파일이 존재해야함)')
            ...
            for j in range(len(json_data["annotations"])):
                ...
                if (str(cls) in args.target_cls) or args.target_cls == "All":
                    ...
                    if os.path.isfile(image_abs_path):
                        # [1] 이미지 img_x, img_y 추출 + 이미지 정보 추출
                        ...
                        # [5] yolo 형식에 맞게 이미지 파일 복사
                        shutil.copy(image_abs_path, target_image_folder_abs_path)

                        # [6] 분류 완료한 이미지 경로 모아두기
                        images_abs_path.append(image_abs_path)
                    # 5] 이미지가 실제로 존재하지 않는 경우
                    else:
                        print(f"No file in local : {image_abs_path}")

                # 6] 출력 : yolo 변환 완료 annotation 개수
